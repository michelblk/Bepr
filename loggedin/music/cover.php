...
if (isset($_GET['songID'])){
    ...
    if (file_exists("../../v3/include/data/music/songcover/".$songID)){
        header('Content-Type: image/jpg');
        if (isset($_GET['size']) && $_GET['size'] == "n"){
            $f = fopen("../../v3/include/data/music/songcover/".$songID, "r");
            $data = fread($f, filesize("../../v3/include/data/music/songcover/".$songID));
            fclose($f);
            echo $data;
            exit;    
        }else{
            if (!file_exists("../../v3/include/data/music/songcover/small/".$songID)){
                $img = resizeImageFromHDD("../../v3/include/data/music/songcover/".$songID, 250, 250);
                imagejpeg($img, "../../v3/include/data/music/songcover/small/".$songID);
                imagejpeg($img);  // Ressourcen!!
                imagedestroy($img); 
                exit;   
            }else{
                $f = fopen("../../v3/include/data/music/songcover/small/".$songID, "r");
                $data = fread($f, filesize("../../v3/include/data/music/songcover/small/".$songID));
                fclose($f);
                echo $data; 
                exit;       
            }
