...
if (isset($_GET['q'])){
    ...
    if (mysql_num_rows($query) != 0){
        while ($song = mysql_fetch_array($query)){
            if (!file_exists("../../include/image/music/songcover/".$song['id'])){$cover = "0";}else{$cover = "1";}
...
if (isset($_GET['orderBy']) && isset($_GET['order'])){
    ...
    $query = mysql_query("SELECT * FROM `music` ORDER BY `".$_GET['orderBy']."` ".$_GET['order']." LIMIT $start, $limit");   //BY id
    $results = array();
    if (mysql_num_rows($query) == 0){
